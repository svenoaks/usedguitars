 ...
 */
    ...
    class ImageResizer {
        ...
        public function resizeToThumb($width, $height) {
            
            if(($this->_width/$this->_height) < ($width/$height)) {
                  $cropedW = $this->_width;
                  $cropedH = ceil($this->_width * $height / $width);
                  $offsetx = 0;
                  $offsety = ($this->_height - $cropedH) / 2;
            } else {
                  $cropedW = ceil($this->_height * $width / $height);
                  $cropedH = $this->_height;
                  $offsetx = ($this->_width - $cropedW) / 2;
                  $offsety = 0;
            }
            ...
            imagefill($newIm, 0, 0, $colorTransparent);
            imagesavealpha($newIm, true);
            imagecopyresampled($newIm, $this->im, 0, 0, $offsetx, $offsety, $width, $height, $cropedW, $cropedH);
            imagedestroy($this->im);
            $this->im = $newIm;
